...
class HomePage
  ...
  def ensure_on_homepage
    if @home_page_url != Capybara.current_session.current_url
      ...
      # mess up country to test 'load_change_country_lightbox'
      mess_up = FALSE
      if mess_up && @page.has_selector?('div#country-popup')
        expr = %Q(div#country-popup a[href*="/Country/Change/AU"])  # cleaner way
        @page.all(:css, "#{expr}").first.click
        puts "!!MESS UP: AFTER CLICKING ON LINK FOR COUNTRY #{@country} : URL = #{Capybara.current_session.current_url}\n"
      end

      # Load the 'Your Location' lightbox if the lightbox is not already displayed
      wait_until(PAGE_ELEMENT_TIMEOUT_SECS) { @page.has_selector?('div#country-popup') }
      load_change_country_lightbox if @page.has_no_selector?('div#country-popup')

      expr = %Q(div#country-popup a[href*="/Country/Change/#{@country}"])  # cleaner way
      @page.all(:css, "#{expr}").first.click
    end
  end
  ...
  def load_change_country_lightbox
    ...
    reload_page_if_not_found(@home_page_url,5) { @page.has_selector?('div#country-popup a') } # reload page if element is not found after 5 secs
  end
